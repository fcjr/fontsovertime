...
for dir in crawl backfill; do
  if as_crawler "exec 9>/tmp/fontsovertime-$dir.lock; flock -n 9 && cd ~/$dir && git pull -q --rebase origin main && pnpm install --frozen-lockfile --silent"; then
    echo "$dir: updated to $(as_crawler "cd ~/$dir && git rev-parse --short HEAD")"
  else
    echo "$dir: a job is running; it picks up the new code when it next starts"
  fi
done
