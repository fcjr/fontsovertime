...
export function trendText(periods: { kind: string; date: string }[], panel: number | null, subject: string) {
  ...
  return {
    ...
    caption: `Dashed lines before ${since} are estimates from web archives. Solid lines are our weekly crawl. The chart follows a fixed set of ${panel} sites, not every site in the rankings.`,
  };
}
